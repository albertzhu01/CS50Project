...
def home():
    ...
    avatar = db.execute("SELECT avatar FROM users WHERE user_id = :user_id", user_id=session["user_id"])
...
def list():
    if request.method == "GET":
        ...
        item = db.execute("SELECT name FROM todos WHERE user=:user AND id=:complete_id", user=session["user_id"], complete_id=complete_id)
...
def settings():
    if request.method == "POST" and request.form.get("profile"):
        ...
        avatar = "crest"
        db.execute("INSERT users SET avatar = :avatar WHERE id = :user_id", avatar=avatar, user_id=session["user_id"])
...
def edit():
    if request.method == "GET":
        ...
        db.execute("UPDATE todos SET priority=:priority, category=:category, addtime=CURRENT_TIMESTAMP WHERE id=:name AND user=:user", name=name, user=session["user_id"], priority=priority, category=category)
        return redirect("/list")
...
def history():
    if request.method == "GET":
        ...
        dropdown = db.execute("SELECT name, id FROM todos WHERE complete=:complete AND user=:user ORDER BY endtime DESC LIMIT 10", complete="true", user=session["user_id"])
        names = []
        ids = []
...
def groups():
    ...
    groupname = db.execute("SELECT groupname FROM studygroup WHERE id = (SELECT groupid FROM users WHERE id = :user_id)", user_id=session["user_id"])
    if groupname:
        groupname = groupname[0]["groupname"]

    data = db.execute("SELECT COUNT(name), SUM(timespent), user FROM todos WHERE complete = :complete AND user IN (SELECT id FROM users WHERE groupid IN (SELECT groupid FROM users WHERE id = :user_id)) GROUP BY user", user_id=session["user_id"], complete="true")
    users = db.execute("SELECT name FROM users WHERE groupid IN (SELECT groupid FROM users WHERE id = :user_id)", user_id=session["user_id"])
    ...
    for i in range(len(data)):
        user_id = data[i]["user"]
        user_name = db.execute("SELECT name FROM users WHERE id=:id", id=session["user_id"])
