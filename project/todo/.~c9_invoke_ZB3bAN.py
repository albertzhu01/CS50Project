...
def list():
    if request.method == "GET":
        ...
        todo_name=item[0]["id"]
        return render_template("completed.html", todo=todo_name, complete=complete_id)
...
def groupmake():
    if request.method == "GET":
        ...
        # insert name and password into database
        db.execute("INSERT INTO studygroup (name, hash) VALUES (:name, :pin)", name=name, pin=generate_password_hash(pin))
        # db.execute("UPDATE users SET groupid = (SELECT id FROM study WHERE name = :name AND pin = :pin) user = :user_id", name=name, pin=generate_password_hash(pin), user_id=session["user_id"])
