...
def list():
    if request.method == "GET":
        ...
        item = db.execute("SELECT name FROM todos WHERE user=:user AND id=:compl", user=session["user_id"], name=complete)
        todo=item[0]["id"]
        return render_template("completed.html", todo=todo, complete=complete_id)
